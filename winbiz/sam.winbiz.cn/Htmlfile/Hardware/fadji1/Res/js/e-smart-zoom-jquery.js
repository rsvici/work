  ...
  $.fn.smartZoom = function(method) {
    ...
    var publicMethods = {
	    ...
	    init : function(options) {
	    	...
	    	var maxscale = 1.2;
	    	var dblclickmaxscale = 0.8;
	    	if (Player.isShowBig) {

	    	    if ($(window).width() <= 500) {
	    	        maxscale = 1.0;
	    	        dblclickmaxscale = 0.8
	    	    }
    ...
    function updateMouseMoveCursor(){
    	...
    	if (parseInt(currentScale * 100) > parseInt(smartData.adjustedPosInfos.scale * 100)) { // multiply by 100 to resolve precision problem
    	    ...
    	    if (!Player.isSpin) {
    	        $(".spinbtn").removeClass("state2");
    	        Player.isSpin = true;
    	    }
    	    targetElement.css({ "cursor": "url(Res/images/Drag.cur),auto" });
    	}
    }
    ...
    function mouseMoveHandler(e){
        ...
        if (Player.isSpin) {
            ...
            //if (Math.abs(smartData.adjustedPosInfos.height - targetRect.height) <= 10 ||  Math.abs(smartData.adjustedPosInfos.width - targetRect.width) <= 10) {
            //    $(".spinbtn").removeClass("state2");
            //    Player.isSpin = true;
            //}
        }
		...
		var currentFirstTouchEv = touchListEv[0];
		    ...
		    if (nbTouch == 1 && !smartData.touch.touchPinch && smartData.settings.touchMoveEnabled == true) { // if the user use only one finger and touchPinch==false => we manage drag
		        ...
		        if (Player.isSpin) {
		            ...
		            Player.movePlay(currentFirstTouchEv);
		        } else {
    ...
    function animate(target, globalLeft, globalTop, width, height, duration, callback){
    	...
    	var smartData = targetElement.data('smartZoomData');
 		var parentOffset = smartData.containerDiv.offset();
   		...
   		$(".watermask").css({ "left": left + 10, "top": top + 10 });
